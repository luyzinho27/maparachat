...
function resolveFirebaseConfig() {
  const candidate = window.MAPARACHAT_FIREBASE_CONFIG;
  if (candidate && typeof candidate === 'object') {
    return candidate;
  }

  // Compatibilidade: permite que uma variável global genérica seja usada.
...
if (!firebaseConfig) {
  console.error(
    'Configuração do Firebase ausente. Crie public/config.js a partir de public/config.example.js.'
  );
  ...
  try {
    ...
    banner.innerHTML =
      ...
      '<h2 style="margin:0 0 12px">Configuração ausente</h2>' +
      '<p style="margin:0 0 12px;opacity:.9">Este app precisa da configuração do Firebase para funcionar.</p>' +
      '<p style="margin:0;opacity:.9">Copie <code style="background:rgba(255,255,255,.12);padding:2px 6px;border-radius:6px">public/config.example.js</code> para <code style="background:rgba(255,255,255,.12);padding:2px 6px;border-radius:6px">public/config.js</code> e preencha seus dados.</p>' +
      '</div>';
  ...
  throw new Error('Firebase config missing (MAPARACHAT_FIREBASE_CONFIG).');
}
